...
function createiFrame(request, response, storageName, browserName, testName, subtestName, testConfig, storageConfig, uniqueID, key) {
    let operation = '';
    ...
    let domainList = [];
    for (let i = 0; i < bitsPerVisit; i++) {
        if (storageName == 'hsts') {
            domainList.push(configToDomain('subdomain', i));
        } else {
    ...
    if (testConfig['write']['iFrame'] != null) {
        ...
        let html = `
            <html>
                ...
                <body>
                    <iframe src="${protocol}://${iFrameDomain}/tests/server/${storageName}/${browserName}/${testName}/${subtestName}/iFrame/write"></iframe>
                </body>
            </html>
            `
        response.send(html);
...
let handle = function(request, response, storage, browser, test, subtest, testConfigs) {
    ...
    if('headers' in testConfigs[storageName]){
        ...
        responseHeaders = {
            'Access-Control-Allow-Origin': (request.headers.referer ? new URL(request.headers.referer).origin: request.headers.origin),
    ...
    else if (request.url.includes("/writeResult")) {
    ...
    else if (request.url.includes("/readResult")) {
        if (request.method == 'POST') {
            readResult(request, response, storageName, browserName, testName, subtestName, testConfig, storageConfig, uniqueID);
        } else  if (request.method == 'OPTIONS') {
            response.send("Awesome");
        } 
    }
